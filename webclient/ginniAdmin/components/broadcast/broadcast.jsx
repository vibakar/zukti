...
export default class BroadCast extends React.Component {
    ...
    componentDidMount() {
        ...
        Axios.get(url).then((response) => {
            this.state.messages = response.data.map((msg, index) => {
                ...
                return <MessageView key={index} date={msg.date} username={msg.username} dispData={msgContentView}/>
            });
    ...
    displayInputContent = (username, content, date) => {
        ...
        this.state.messages.unshift(<MessageView key={this.state.messages.length - 1} date={date} username={username} dispData={msgContentView}/>)
        this.setState({messages: this.state.messages});
    }
