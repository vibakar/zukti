    ...
    render() {
      let inputs = this.props.blogs.map((input,index)=>{
        ...
        return (
          ...
          {this.props.blogs.length==1?'':<Button id={index} onClick={this.removeAnswer} style={{marginLeft:'2%',backgroundColor:'white',color:'red'}}><Icon name='minus' circular/></Button>}
          </div>
        )
      })
        return (
