...
export default class AddQuestionCategory extends React.Component {
    ...
    addCategory() {
        ...
        Axios.post(url, {
          ...
          this.setState({categories:this.state.categories});
            ...
            alert('Error in AJAX call while creating a new categorty');
            console.log(error);
        });
    }
    ...
    render() {
        ...
        let existingCategories= this.state.categories.map((item)=>{
          return(
            <Card key={item.nodeID}>
                <Button content={item.categoryName} onClick={() => this.openQuestionSet(item)} circular={true}></Button>
        ...
        let categoriesDisplay= (
            ...
            <Grid.Row>
              <input autoComplete="off" type='text' name='category' ref='category' placeholder='Add new question category'/>
        ...
        return <BuildAi handlerBackToQuestionCategories={this.changeIsCategoryClicked} categoryID={this.state.selectedCategory.nodeID} categoryName={this.state.selectedCategory.categoryName}/>
      }
    }
}
