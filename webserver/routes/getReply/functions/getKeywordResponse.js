    ...
    User.findOne({
        ...
        if (types.length === 0) {
            query = `UNWIND ${JSON.stringify(keywords)} AS token
                 ...
                 ORDER BY CASE WHEN rel.likes=0 AND rel.dislikes=0 THEN rel.likes ELSE (rel.likes/(rel.likes+rel.dislikes)) END DESC                 RETURN LABELS(n)as contentType ,COLLECT(distinct n.value) `;
        }
