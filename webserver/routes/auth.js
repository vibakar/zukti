...
app.get('/verify', function(req, res) {
    ...
    app.post('/forgetpassword', function password(req, res) {
          rand=Math.floor((Math.random() * 100) + 54);
          RegisteredUser.update({email: req.body.email},{$set:{verificationID:rand}},function(err){
             if(err) {
                console.log(err);
                console.log("error :(");
            }
          ...
          RegisteredUser.find({email: req.body.email},function(err,profile){
              if(err) {
                res.send(err);
  ...
  if(req.body)
  {
    ...
    request4= req.body.lastname;
      RegisteredUser.update({'email':request1}, {$set:{'name':request2,'firstname':request3,'lastname':request4}}, function(err) {
      if(err) {
        res.send(err);
      }
      else  {
...
if(req.body)
{
  ...
  RegisteredUser.update({'email':request1}, {$set:{'password':request2}}, function(err) {
    if(err) {
      res.send(err);
    }
    else  {
